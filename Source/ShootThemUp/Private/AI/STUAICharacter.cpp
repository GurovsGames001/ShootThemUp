...
void ASTUAICharacter::OnHealthChanged(float Health, float HealthDelta)
{
    ...
    const auto HealthWidgetBar = Cast<USTUHealthBarWidget>(HealthWidgetComponent->GetUserWidgetObject());
    if (!HealthWidgetBar) return;

    HealthWidgetBar->SetHealthPercent(HealthComponent->GetHealthPercent());
}
